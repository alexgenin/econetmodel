...
#define remaining_eq_value  2.0;

// Print debug messages ?
static int dbg = 0;
static double zerotol = 1e-8;     // minimum abundance of a species
static double eq_zerotol = 1e-30; // max derivs value to consider eq
static int    steps_since_root = min_step_btw_root;  // min number of evaluations to consider 
                                                        // btw two equilibrium
...
// // Generated by R (Wed Mar  4 18:50:01 2015)
#define Nsp 8
#define Np 225
 ...
 * errors.
 */
double remaining_eq = remaining_eq_value;
...
void remove_species(int *n, double *t, double *y) { 
  ...
  long int which_species = lrint(p.removed_species[0]) - 1;
//   Rprintf("Removing species %i \n", which_species + 1);
  
  y[which_species] = 0.; // OMG dead stuff
}
...
//       }
  ...
  // Store max absolute derivative
  for (int i=1; i<*neq; i++) {
    yout[0] = fmaxf(ydot[i-1], fabs(ydot[i]));
    yout[1] = remaining_eq;
  }
