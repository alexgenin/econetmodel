...
# Generate all values declaration given a plist
gen_declarations <- function(plist, collapser='\n') { 
  out <- NULL
  for (i in seq.int(length(plist))) { 
    out <- append(out, gen_onedeclaration(plist[i]))
  }
  
  paste(out,collapse=collapser)
}

# Generate a value declaration
gen_onedeclaration <- function(list, 
                            type.str = 'double',
                            suffix   = ';') {
  
  # Get name for element
  name <- names(list)
  size <- vecmat_switch(list[[1]],'[Nsp]','[Nsp][Nsp]')
  
  # Return string
  paste0(type.str, ' ', name, size, suffix)
}
